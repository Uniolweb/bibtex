...
// Need to figure this one out for RTF
             ...
             */
                ...
                if ($this->format == 'sxw') {
                    ...
                    $data = str_replace(">", "&gt;", $data);
                    $data = preg_replace("/&(?![a-zA-Z0-9#]+?;)/", "&amp;", $data);
                    $data = preg_replace("/\[b\](.*?)\[\/b\]/is",
                        "<text:span text:style-name=\"textbf\">$1</text:span>", $data);
                    $data = preg_replace("/\[i\](.*?)\[\/i\]/is", "<text:span text:style-name=\"emph\">$1</text:span>",
                        $data);
                    $data = preg_replace("/\[sup\](.*?)\[\/sup\]/is",
                        "<text:span text:style-name=\"superscript\">$1</text:span>", $data);
                    $data = preg_replace("/\[u\](.*?)\[\/u\]/is",
