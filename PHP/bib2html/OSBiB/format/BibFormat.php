...
class BibFormat
{
    ...
    protected ?\STYLEMAPBIBTEX $styleMap = null;
    ...
    public function loadStyle($stylePath, $style)
    {
        ...
        $uc = $stylePath . strtolower($style) . '/' . strtoupper($style) . '.xml';
     ...
     * @version    1
     */
    public function getStyle($common, $types)
    {
    ...
    public function preProcessBibtex(&$row, $type)
    {
         ...
         */
             ...
             */
                ...
                if (($type == 'miscellaneous') && array_key_exists('howpublished', $row)) {
    ...
    public function map($template = false)
    {
        ...
        foreach ($this->$type as $key => $value) {
            ...
            if (isset($this->$pluralKey) && $this->$pluralKey) { // plural alternative for this key
                ...
                if (isset($this->$pluralKey)) { // singular alternative for this key
     ...
     * @version    1
     */
    public function formatNames($creators, $nameType, $citation = false)
    {
        ...
        foreach ($creators as $creator) {
            ...
            if ($style[$nameStyle] == 0) { // Joe Bloggs
                ...
                if ($style[$nameStyle] == 1) { // Bloggs, Joe
                    ...
                    if ($style[$nameStyle] == 2) { // Bloggs Joe
                        $nameString =
                                stripslashes($creator['prefix']) . ' ' .
                                stripslashes($creator['surname']) . ' ' .
                                $firstName;
        ...
        if ($nameType == 'creator1') {
            ...
            if (($style['primaryCreatorRepeat'] == 2) && ($this->previousCreator == $pString)) {
                ...
                if (($style['primaryCreatorRepeat'] == 1) &&
    ...
    public function checkInitials(&$creator, $initialsStyle, $firstNameInitial)
    {
        ...
        if ($creator['firstname'] && !$firstNameInitial) { // Full name
            ...
            if ($creator['firstname']) { // Initial only of first name.  'firstname' field may actually have several 'firstnames'
                $fn = explode(' ', stripslashes($creator['firstname'])); // 2011-02-22 VB: explode statt split
                $firstTime = true;
                foreach ($fn as $name) {
                    if ($firstTime) {
                        $firstNameInitialMake = $this->utf8->utf8_strtoupper($this->utf8->utf8_substr(
                            trim($name),
                            0,
                            1
                        ));
    ...
    public function formatRunningTime($minutes, $hours)
    {
        ...
        if ($this->style['runningTimeFormat'] == 0) { // 3'45"
            ...
            if ($this->style['runningTimeFormat'] == 1) { // 3:45
                if (isset($minutes) && $minutes) {
                    if ($minutes < 10) {
                        $minutes = '0' . $minutes;
                    }
                ...
                if ($this->style['runningTimeFormat'] == 1) { // 3,45
                    ...
                    if ($this->style['runningTimeFormat'] == 3) { // 3 hours, 45 minutes
                        ...
                        if ($this->style['runningTimeFormat'] == 4) { // 3 hours and 45 minutes
                            $hours = ($hours == 1) ? $hours . ' hour' : $hours . ' hours';
                            if (isset($minutes) && $minutes) {
                                $minutes = ($minutes == 1) ? $minutes . ' minute' : $minutes . ' minutes';
    ...
    public function formatDate($startDay, $startMonth, $endDay, $endMonth)
    {
        ...
        if ($this->style['monthFormat'] == 1) { // Full month name
            ...
            if ($this->style['monthFormat'] == 2) { // User-defined
                for ($i = 1; $i <= 12; $i++) {
    ...
    public function formatEdition($edition)
    {
        ...
        if (!is_numeric($edition)) {
            ...
            if ($this->style['editionFormat']) { // 10th
     ...
     * @version    1
     */
    public function cardinalToOrdinal($cardinal)
    {
        $modulo = $cardinal % 100;
