...
// Transform XML nodal array to resource type template strings
    public function arrayToTemplate($types)
    {
        $this->fallback = [];
...
// display the citation templating form
    public function displayCiteForm($type)
    {
        ...
        $example = [$this->messages->text('cite', 'printCreator'),
        ...
        $td = FORM::selectedBoxValue(
            $this->messages->text('cite', 'consecutiveCreator'),
            'cite_consecutiveCreator',
            $example,
            $consecutive,
            2
        );
        ...
        $td .= MISC::P(FORM::checkbox(
            ...
            'cite_ambiguousName',
            $ambiguousName
        ) . ' ' . $this->messages->text('cite', 'ambiguousFull'));
        $td .= MISC::P(FORM::checkbox(
            false,
            'cite_ambiguousMore',
            $ambiguousMore
        ) . ' ' . $this->messages->text('cite', 'ambiguousMore'));
        $td .= MISC::P(FORM::checkbox(
            false,
            'cite_ambiguousTitle',
            $ambiguousTitle
        ) . ' ' . $this->messages->text('cite', 'ambiguousTitle'));
        $td .= MISC::P(FORM::checkbox(
            false,
            'cite_ambiguousYear',
            $ambiguousYear
        ) . ' ' . $this->messages->text('cite', 'ambiguousYear'));
        $pString .= TABLE::td($td, false, false, 'top');
        $example = [$this->messages->text('style', 'creatorFirstNameFull'),
            'T. U. ', 'T.U.', 'T U ', 'TU'];
        $td = FORM::selectedBoxValue(
            $this->messages->text('cite', 'ambiguousNameFormat'),
            'cite_ambiguousNameFormat',
            $example,
            $nameFormat,
            5
        );
        $pString .= TABLE::td($td);
        $example = ['1999a, b', '1999a, 1999b'];
        $td = FORM::selectedBoxValue(
            $this->messages->text('cite', 'ambiguousYearFormat'),
            'cite_ambiguousYearFormat',
            $example,
            $yearFormat,
            2
        );
        ...
        $example = [$this->messages->text('cite', 'footnoteStyleBib'),
        ...
        $td .= FORM::checkbox(
            ...
            'cite_ibidPage',
            $ibidPage
        ) . ' ' . $this->messages->text('cite', 'ibidPage');
        $td .= MISC::br() . '&nbsp;' . MISC::br();
        ...
        $example = [$this->messages->text('cite', 'footnoteCitationPageFormatNever'),
        ...
        $td = FORM::selectedBoxValue(
            $this->messages->text('cite', 'footnoteCitationPageFormat'),
            'cite_footnoteCitationPageFormat',
            $example,
            $pageFormat,
            3
        );
        ...
        $td .= $this->messages->text('cite', 'template') . ' ' .
            ...
            MISC::br() . 'pages', 'small');
        $td .= MISC::br() . '&nbsp;' . MISC::br();
        $example = [$this->messages->text('cite', 'footnotePageAfter'),
            $this->messages->text('cite', 'footnotePageBefore')];
        $pagePosition = base64_decode($this->session->getVar('cite_footnotePagePosition'));
        $td .= FORM::selectedBoxValue(
            $this->messages->text('cite', 'footnotePagePosition'),
            'cite_footnotePagePosition',
            $example,
            $pagePosition,
            2
        );
    ...
    public function displayStyleForm($type)
    {
        ...
        foreach ($types as $key) {
            ...
            $pString .= TABLE::td(FORM::textareaInput(
                $this->messages->text('resourceType', $key),
                $keyName,
                $input,
                80,
                3
...
// parse input into array
    public function parseStringToArray($type, $subject, $map = false, $preview = false)
    {
        if (!$subject) {
            return [];
        }
